...
namespace Confuser.Protections.Constants {
	internal class InjectPhase : ProtectionPhase {
		...
		protected override void Execute(ConfuserContext context, ProtectionParameters parameters) {
			...
			var random = context.Registry.GetService<IRandomService>().GetRandomGenerator(Parent.Id);
			...
			moduleCtx.DecoderCount = parameters.GetParameter(context, context.CurrentModule, "decoderCount", random.NextInt32(1, 10));
			...
			//Get the runtime decompressor
			MethodDef decompressor = compression.GetRuntimeDecompressor(context.CurrentModule, member => {
			...
			MutateInitializer(moduleCtx, decompressor);
		...
		void InitialiseDataField(ConfuserContext context, CEContext moduleCtx) {
		...
		void InjectHelpers(ConfuserContext context, IRuntimeService rt, CEContext moduleCtx) {
			...
			IEnumerable<IDnlibDef> members = InjectHelper.Inject(
				rt.GetRuntimeType("Confuser.Runtime.Constant"),
				context.CurrentModule.GlobalType,
				context.CurrentModule);

			foreach (IDnlibDef member in members) {
				if (member.Name == "GetString") {
			...
			InitialiseDataField(context, moduleCtx);

			MethodDef decoder = rt.GetRuntimeType("Confuser.Runtime.Constant").FindMethod("GetString");
		...
		void MutateInitializer(CEContext moduleCtx, MethodDef decompress) {
