...
namespace Confuser.Protections.ControlFlow {
	internal class IfMangler : ManglerBase {
		struct Trace {
		...
		public override void Mangle(CilBody body, ScopeBlock root, CFContext ctx) {
			...
			var remLocal = new Local(ctx.Method.Module.CorLibTypes.UInt32);
			body.Variables.Add(remLocal);
			...
			foreach (InstrBlock block in GetAllBlocks(root)) {
				...
				switchHdr.Add(Instruction.Create(OpCodes.Stloc, remLocal));

				void AddJumpTableTarget(Instruction target, int keyIdentifier) {
					switchHdr.Add(Instruction.Create(OpCodes.Ldloc, remLocal));

					var stateVar = new Local(ctx.Method.Module.CorLibTypes.Int32);
					body.Variables.Add(stateVar);
					if(ctx.Random.NextBoolean())
						ctx.DynCipher.GenerateExpressionMutation(ctx.Random, ctx.Method, stateVar, switchHdr, keyIdentifier, ctx.Random.NextInt32(1, ctx.Depth));
					else
						ctx.DynCipher.GenerateStatementMutation(ctx.Random, ctx.Method, stateVar, switchHdr, keyIdentifier, ctx.Random.NextInt32(1, 3));

					switchHdr.Add(Instruction.Create(OpCodes.Beq, target));
				...
				while (current.Next != null) {
					...
					if (i != 0) {
						...
						if (newStatement.Last().IsBr()) {
							...
							if (!trace.IsBranchTarget(newStatement.Last().Offset) &&
								...
								AddJumpTableTarget(newStatement[0], keyId[i]);
					...
					{
						AddJumpTableTarget(switchHdr[0], keyId[i]);
				...
				AddJumpTableTarget(current.Value[0], keyId[i]);
