...
namespace Confuser.Protections.AntiTamper {
	internal class AntiMode : IModeHandler {
		...
		public void HandleInject(AntiTamperProtection parent, ConfuserContext context, ProtectionParameters parameters) {
			...
			var dynCipher = context.Registry.GetService<IDynCipherService>();

			Local stateVariable = new Local(initMethod.Module.CorLibTypes.Int32);
			initMethod.Body.Variables.Add(stateVariable);

			MutationHelper.InjectKeys(initMethod,
									  new[] { 0, 1, 2, 3, 4 },
									  new[] { (int)(name1 * name2), (int)z, (int)x, (int)c, (int)v },
									  (instr => {
										  IList<Instruction> generatedMutation = new List<Instruction>();
										  dynCipher.GenerateStatementMutation(random,
											  initMethod,
											  stateVariable,
											  generatedMutation,
											  (int)instr.Operand,
										  ...
										  instr.ReplaceWithInstructionList(generatedMutation, initMethod);
									  }));
			...
			int randValue = random.NextInt32(0, int.MaxValue);
