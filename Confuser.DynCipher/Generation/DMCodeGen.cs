	...
	public class DMCodeGen {
		...
		void EmitStatement(Statement statement) {
			if (statement is AssignmentStatement) {
				...
				Instruction start = OpCodes.Ldloc.ToInstruction(local);
				Instruction exit = OpCodes.Nop.ToInstruction();
				Instruction statementBegin = OpCodes.Nop.ToInstruction();
				...
				Emit(OpCodes.Blt.ToInstruction(statementBegin));
				Emit(OpCodes.Br.ToInstruction(exit));
				Emit(statementBegin);
				...
				Emit(OpCodes.Br.ToInstruction(start));
				...
				Label statementBeginLabel = ilGen.DefineLabel();
				...
				ilGen.Emit(OpCodes.Blt, statementBeginLabel);
				ilGen.Emit(OpCodes.Br, exitLabel);
				ilGen.MarkLabel(statementBeginLabel);
				...
				ilGen.Emit(OpCodes.Br, startLabel);
